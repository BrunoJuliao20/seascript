...
REG_SEA = Region(455,166,1012,752)
...
IMG_DESTROCO2 = "IMG_SIRIU.png"
IMG_EVENTCHEST = Pattern("IMG_EVENTCHEST.png").similar(0.50)
...
LAST_MOVE = None
LAST_POS = None
CHECK_MOVE_TIMEOUT = datetime.datetime.now() + datetime.timedelta(seconds=2)
...
REG_POS_01 = Region(1562,109,12,30)
REG_POS_02 = Region(1559,152,9,37)
REG_POS_03 = Region(1576,180,46,10)
REG_POS_04 = Region(1590,128,21,19)
REG_POS_05 = Region(1618,150,18,40)
# End
...
def kill_npc(e):
    ...
    if BUSY == True:
        ...
        try:
            ...
            click(location)
            rightClick(Location(location.x, (location.y + 40)))
...
def kill_npc2(e):
    ...
    if BUSY == True:
        ...
        current_atk = Screen(0).capture(REG_ATK)
        try:
            match = REG_SEA.find(Pattern(IMG_DESTROCO2).similar(SIM_IMG_DESTROCO2).targetOffset(0, -26))
            location = match.getTarget()
            click(location)
...
REG_SEA.onAppear(Pattern(IMG_DESTROCO).similar(SIM_IMG_DESTROCO), kill_npc)
REG_SEA.onAppear(Pattern(IMG_DESTROCO2).similar(SIM_IMG_DESTROCO2), kill_npc2)
