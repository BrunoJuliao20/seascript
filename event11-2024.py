...
IMG_NPC1 = "NPC_LOLLIERE.png"
IMG_NPC2 = "NPC_MERRIERE.png"
IMG_NPC3 = "NPC_BJOR.png"
...
IMG_BRILHO = "IMG_BRILHO.png"
IMG_PRESENTE = "IMG_PRESENTE.png"
...
SIM_IMG_NPC5 = 0.8
SIM_IMG_NPC6 = 0.7
SIM_IMG_EVENTCHEST = 0.6
SIM_IMG_CHESTFREE = 0.7
SIM_IMG_BRILHO = 0.7
SIM_IMG_PRESENTE = 0.75
SIM_IMG_LOW_HP = 0.75
SIM_IMG_ATK = 0.6
SIM_IMG_RADAR = 0.85
    ...
    if BUSY == True:
        ...
        try:
            ...
            for i in range(8):
                wait(2)
                if REG_ATK.exists(Pattern(IMG_DARK).similar(0.99)):
                    max_coleta_tempo = datetime.datetime.now() + datetime.timedelta(seconds=12)  
                    while datetime.datetime.now() < max_coleta_tempo:
                        try:
                            match = REG_SEA.find(Pattern(IMG_PRESENTE).similar(SIM_IMG_PRESENTE))
                            distancia = sqrt((pos_caixa.x - player.x)**2 + (pos_caixa.y - player.y)**2)
                            location = match.getTarget()
                            click(location)  
                            wait(2)  
...
def collect_BRILHO(e):
    ...
    if BUSY == True:
        ...
        type("9")
        try:
            match = REG_SEA.find(Pattern(IMG_BRILHO).similar(SIM_IMG_BRILHO))
            location = match.getTarget()
            rightClick(location)
            wait(5)
            end_wait = False
            ...
            while end_wait == False:
                if wait_region.exists(Pattern(IMG_BRILHO).similar(SIM_IMG_BRILHO)):
                    pass
                else:
                    end_wait = True
...
#REG_SEA.onAppear(Pattern(IMG_BRILHO).similar(SIM_IMG_BRILHO), collect_BRILHO)
