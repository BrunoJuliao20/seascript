...
REG_POS_02 = Region(1613,118,40,33)
REG_POS_03 = Region(1622,130,14,32)
...
def kill_npc(e):
    ...
    if BUSY == True:
        ...
        try:
            match = REG_SEA.find(Pattern(IMG_NPC).similar(SIM_IMG_NPC1).targetOffset(0, -30))
            location = match.getTarget()
            click(location)
            click(Location(location.x, (location.y + 30)))
            ...
            while datetime.datetime.now() < atk_timeout:
                ...
                if REG_ATK.exists(Pattern(current_atk).similar(0.6)):         
                    ...
                    wait(Pattern(current_atk).similar(0.6), NPC_WAIT)
                    break
